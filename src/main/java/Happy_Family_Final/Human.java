...
public abstract class Human {
    ...
    public Human(String firstName, String lastName, long birthDate, int iq, Human mother, Human father) {
        this.name = firstName;
    ...
    public String toString() {
        ...
        String str=" ";
        if (schedule!=null){
            str = String.valueOf(schedule.values());
        }
        return "{"+getGender()+"|"+name+"|"+surname+"|"+
                birthDate + "|"+IQ+
                "|"+mother+"|"+father+"|"+str+"}";
    }
    ...
    public int getAge() {
        LocalDate date1 = new Date(this.birthDate).toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        LocalDate date2 = LocalDate.now();
        System.out.println(date1 + " "+ date2);
        Period period = date2.until(date1);
//        System.out.println(this.name + " is "+Math.abs(period.getYears())+" years, "+Math.abs(period.getMonths())+" months "+
